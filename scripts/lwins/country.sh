...
docker exec $SERVICE_NAME psql -U wine -d wine_db -c "INSERT INTO countries (name) SELECT DISTINCT country FROM lwins WHERE country IS NOT NULL AND country <> '' ON CONFLICT (name) DO NOTHING;"
docker exec $SERVICE_NAME psql -U wine -d wine_db -c "SELECT id, name FROM countries"
