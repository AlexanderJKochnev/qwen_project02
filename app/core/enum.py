...
def fetch_all_startup_data() -> dict:
    ...
    with SessionLocalSync() as session:
        # Запрос 1: Модели
        models = session.scalars(select(Ollama.model).order_by(Ollama.model.asc()).scalars()).all()
        # models = session.execute(text("SELECT name FROM llm_models")).scalars().all()
        data['models'] = [m for m in models] or ["qwen3:8b"]
        # Запрос 2: Prompt
        prompts = session.scalars(select(Prompt.role).order_by(Prompt.role.asc()).scalars()).all()
        data['prompts'] = [pr for pr in prompts] or ["translator"]

        # Запрос 3: Preset
        presets = session.scalars(select(Proption.preset).order_by(Proption.preset.asc()).scalars()).all()
