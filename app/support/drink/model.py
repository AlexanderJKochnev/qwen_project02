...
create_gin_index_sql = DDL("""
    CREATE INDEX drink_trigram_idx_combined ON drinks
        USING gin (
            (coalesce(title, '') || ' ' ||
             coalesce(title_ru, '') || ' ' ||
             coalesce(title_fr, '') || ' ' || coalesce(title_es, '') || ' ' || 
             coalesce(subtitle, '') || ' ' ||
             coalesce(subtitle_ru, '') || ' ' ||
             coalesce(subtitle_fr, '') || ' ' || coalesce(subtitle_es, '') || ' ' || 
             coalesce(description, '') || ' ' ||
             coalesce(description_ru, '') || ' ' ||
             coalesce(description_fr, '') || ' ' || coalesce(description_es, '') || ' ' || 
             coalesce(recommendation, '') || ' ' ||
             coalesce(recommendation_ru, '') || ' ' ||
             coalesce(recommendation_fr, '') || ' ' || coalesce(recommendation_es, '') || ' ' || 
             coalesce(madeof, '') || ' ' ||
             coalesce(madeof_ru, '') || ' ' ||
             coalesce(madeof_fr, '') || ' ' || coalesce(madeof_es, ''))
            gin_trgm_ops
        );
    """)
