...
2. при первом запуске (если потом не сделаю автозагрузчик):
   1. docker exec -it ollama-ollama-1 ollama pull translategemma  # модель 2b дообученная
   ...
   5. docker exec -it ollama-ollama-1 ollama pull deepseek-r1:7b
   6. docker exec -it ollama ollama list # проверка какие модели загружены
