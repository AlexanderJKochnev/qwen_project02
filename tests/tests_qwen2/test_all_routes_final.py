...
class EfficientRouteTestManager:
    ...
    def generate_basic_test_data(self, path: str) -> Dict[str, Any]:
        ...
        return {
            "name": f"test_{entity}_{timestamp}",
            "description": f"Test {entity}"
        }
            ...
            async with semaphore:
                ...
                if has_id_param and self.created_items:
                    ...
                    for created_path, items in self.created_items.items():
                        if items and created_path.replace('{id}', '').replace('{item_id}', '').replace('{_id}', '') in route_path.replace('{id}', '').replace('{item_id}', '').replace('{_id}', ''):
                            item_id = items[0]
        ...
        async def test_single_patch(route):
            async with semaphore:
                ...
                if not matched:
                    results[route_path] = {
                        'method': 'PATCH',
                        'status': 'SKIPPED - No matching item',
                        'status_code': None,
                    }
...
async def test_all_routes_efficient(authenticated_client_with_db, test_mongodb):
    ...
    efficiency = ((sum(method_totals.values()) - saved_routes) / sum(method_totals.values())) * 100 if sum(method_totals.values()) > 0 else 0
