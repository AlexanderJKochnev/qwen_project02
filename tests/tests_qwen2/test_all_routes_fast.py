...
class FastRouteTestManager:
    ...
    def generate_basic_test_data(self, path: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "description": f"Test data for {path.split('/')[-1]}"
        }
        ...
        async def test_single_get(route):
            async with semaphore:
                ...
                if has_id_param and self.created_items:
                    ...
                    matching_base = next((base for base in self.created_items.keys() 
                                        if base.replace('{id}', '').replace('{item_id}', '').replace('{_id}', '')
                                        in route_path.replace('{id}', '').replace('{item_id}', '').replace('{_id}', '')), None)
                    ...
                    if matching_base and self.created_items[matching_base]:
                        ...
                        result = await self.make_request_with_timeout(authenticated_client_with_db, 'GET', route_path.replace('{id}', '1').replace('{item_id}', '1').replace('{_id}', '1'))
                        results[route_path] = {
                            'method': 'GET (single, mock id)',
        ...
        async def test_single_patch(route):
            async with semaphore:
                ...
                if matching_base and self.created_items[matching_base]:
                    ...
                    results[route_path] = {
                        'method': 'PATCH',
                        'status': 'SKIPPED - No items to update',
                        'status_code': None,
                    }
        ...
        async def test_single_delete(route):
            async with semaphore:
                ...
                if matching_base and self.created_items[matching_base]:
                    ...
                    results[route_path] = {
                        'method': 'DELETE',
                        'status': 'SKIPPED - No items to delete',
                        'status_code': None,
                    }
